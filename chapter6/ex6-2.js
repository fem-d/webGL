    ...
    function(S, Node, WebGLApp, Program, Camera, CameraInteractor, Scene, SceneTransforms, Light, Lights, Floor, Axis) {
        ...
        var main = {
            ...
            configure: function() {
                ...
                gl.uniform3fv(prg.uPositionRedLight, red.position);
                gl.uniform3fv(prg.uPositionGreenLight, green.position);
                gl.uniform3fv(prg.uPositionBlueLight, blue.position);

                gl.uniform4fv(prg.uDiffuseRedLight, red.diffuse);
                gl.uniform4fv(prg.uDiffuseGreenLight, green.diffuse);
                gl.uniform4fv(prg.uDiffuseBlueLight, blue.diffuse);

                gl.uniform1f(prg.uCutOff, 0.4);
                ...
                try {
                    ...
                    for (var i = 0; i < objects.length; i++) {
                        ...
                        if (object.alias == 'light1') {
                            mat4.translate(transforms.mvMatrix, gl.getUniform(prg, prg.uPositionRedLight));
                            object.diffuse = gl.getUniform(prg, prg.uDiffuseRedLight);
                            gl.uniform1i(prg.uLightSource, true);
                        }

                        if (object.alias == 'light2') {
                            mat4.translate(transforms.mvMatrix, gl.getUniform(prg, prg.uPositionGreenLight));
                            object.diffuse = gl.getUniform(prg, prg.uDiffuseGreenLight);
                            gl.uniform1i(prg.uLightSource, true);
                        }

                        if (object.alias == 'light3') {
                            mat4.translate(transforms.mvMatrix, gl.getUniform(prg, prg.uPositionBlueLight));
                            object.diffuse = gl.getUniform(prg, prg.uDiffuseBlueLight);
                            gl.uniform1i(prg.uLightSource, true);
                        }
